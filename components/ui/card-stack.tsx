...
export type CardStackProps<T extends CardStackItem> = {
  ...
  /**
   * Apply subtle per-card rotation and Y-position jitter so cards don't sit
   * perfectly on top of each other — creates an "imperfect, chaotic stack" look.
   * Uses fixed offsets (not Math.random) for deterministic, consistent renders.
   * Default: false
   */
...
// Fixed per-card jitter offsets used when `randomOffsets` prop is true.
        ...
        >
          <AnimatePresence initial={false}>
            {items.map((item, i) => {
              ...
              // Per-card jitter: adds a small, fixed rotation and Y offset to each card
