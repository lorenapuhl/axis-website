"use client"
// BookingsDashboard — split view: left shows class list, right shows bookings for selected class.
// Each booking has a status badge and action buttons (Confirm, Waitlist, Remove).
...
// Mock data — realistic booking records for each class
...
export default function BookingsDashboard() {
  // selectedClass — which class is shown in the right panel
  const [selectedClass, setSelectedClass] = useState<ClassItem>(CLASSES[0])
  // bookingStates — tracks local changes (confirm/waitlist/remove) without real backend
  const [bookingStates, setBookingStates] = useState<Record<string, Booking["status"]>>({})

  // Get effective status — local override takes priority over original data
  ...
  return (
    <div className="bg-zinc-950 rounded-2xl border border-zinc-900 h-96 flex overflow-hidden">
      ...
      {/* ── RIGHT: BOOKINGS FOR SELECTED CLASS ── */}
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="px-4 py-2.5 border-b border-zinc-900 flex items-center justify-between shrink-0">
          ...
          <span className="text-zinc-600 text-xs font-instrument">
            {selectedClass.bookings.filter((b) => getStatus(b) === "confirmed").length} confirmed
          </span>
        </div>
